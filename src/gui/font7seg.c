...
void font7segWriteChar(char code)
{
    ...
    if (code >= '0' && code <= '9') {
        ...
        const uint8_t *digStart = &font7seg[F7_END + 7 * (font7seg[F7_THICK] * 2 + 1)];
        uint8_t startLine = *digStart++;
        uint8_t point1, point2;
        for (uint8_t line = 0; line < font7seg[F7_THICK]; line++) {
            point1 = *digStart++;
            point2 = *digStart++;
            int16_t y;
            if (code == '.') {
                ...
                y = glcd->y + startLine - 2 * font7seg[F7_THICK] + line;
                glcdDrawLine(glcd->x + point1, y, glcd->x + point2, y, glcd->fontFg);
                y = glcd->y + 2 * font7seg[F7_THICK] + line;
                glcdDrawLine(glcd->x + point1, y, glcd->x + point2, y, glcd->fontFg);
            }
        }
        glcdSetXY(glcd->x + 2 * font7seg[F7_THICK], glcd->y);
        return;
    ...
    for (uint8_t seg = 0; seg < 7; seg++) {
        ...
        const uint8_t *digStart = &font7seg[F7_END + seg * (font7seg[F7_THICK] * 2 + 1)];
        uint8_t startLine = *digStart++;
        uint8_t point1, point2;
        for (uint8_t line = 0; line < font7seg[F7_THICK]; line++) {
            point1 = *digStart++;
    ...
    glcdSetXY(glcd->x + font7seg[F7_WIDTH] + font7seg[F7_THICK], glcd->y);
}
